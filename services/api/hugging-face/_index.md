...
1. **冷启动延迟:** 模型首次请求或长时间未使用后，需要加载时间（可能 10-30 秒）
2. **速率限制:** 免费用户有严格的速率限制，超过会返回 429 错误
3. **模型可用性:** 部分模型可能需要 PRO 账户或特殊权限
4. **无 SLA 保证:** 免费层不提供服务水平协议保证
5. **生产使用限制:** 不建议免费层用于生产环境，建议使用专用推理端点
...
A: 有免费层级（每小时约数百次请求），也可升级到 PRO 账户（$9/月）获得更高配额。
